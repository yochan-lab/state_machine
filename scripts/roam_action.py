...
class RoamServer(object):
    ...
    def __init__(self, name):
        ...
        self._server = SimpleActionServer("roam", state_machine.msg.roamAction, execute_cb=self.roam, auto_start = False)
        self._server.start()
    ...
    def roam(self, goal):
        ...
        #while self._client.get_state() != GoalStatus.SUCCEEDED: #check movebase success.
        #    #check for premption
        #    self._feedback.there_yet = False
        #    if self._server.is_preempt_requested():
        #        rospy.loginfo('%s: Preempted' % self._action_name)
